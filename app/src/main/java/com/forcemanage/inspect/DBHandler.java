...
import com.forcemanage.inspect.attributes.Template_Attributes;
...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_TEMPLATE_TABLE = "CREATE TABLE " +
                TABLE_TEMPLATE + "("
                + COLUMN_CLIENT_NAME + " STRING,"
                + COLUMN_TEMPLATE_NAME + " STRING,"
                + COLUMN_CAT_ID + " INTEGER,"
                + COLUMN_LEVEL + " INTEGER,"
                + COLUMN_PARENT + " INTEGER,"
...
//        db.delete(TABLE_PREFERENCE, null, null);
  ...
  */
    ...
    public void  update_TEMPLATE_FromServer(Template_Attributes template_attributes) {

        //replace will delete the row if the category already exists
        ContentValues values = new ContentValues();
        values.put(COLUMN_CLIENT_NAME, template_attributes.getClientName());
        values.put(COLUMN_TEMPLATE_NAME, template_attributes.getTemplateName());
        values.put(COLUMN_CAT_ID, template_attributes.getCatId());
        values.put(COLUMN_LEVEL, template_attributes.getLevel());
        values.put(COLUMN_PARENT, template_attributes.getParent());
        values.put(COLUMN_LABEL, template_attributes.getLabel());
        values.put(COLUMN_CHILD, template_attributes.getChild());
        values.put(COLUMN_A_ID, template_attributes.getaId());
        SQLiteDatabase db = this.getWritableDatabase();
        //replace will delete the row if the OR already exists

        db.replace(TABLE_TEMPLATE, null, values);
        db.close();
    ...
    public void updateInspectionItemPhoto_insert(int projId, int iId, int aId, String Image, int Img) {
        ...
        switch (Img){
            ...
            case 2:
                ...
                break;
            case 3:
                contentValues.put(COLUMN_IMG3, Image);
                break;
            case 4:
                contentValues.put(COLUMN_IMG4, Image);
                break;
            case 5:
                contentValues.put(COLUMN_IMG5, Image);
                break;
            case 6:
                contentValues.put(COLUMN_IMG6, Image);
                break;
        }
    ...
    public void updateInspectionPhoto(int projectId, int iId, String photo) {
        // Open a database for reading and writing
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void saveTemplate(String client, String name, int projId){
        SQLiteDatabase db = this.getWritableDatabase();
        String selectQuery;
        Cursor cursor;
        ...
        if (cursor.moveToFirst()) {
            do {
                ContentValues values = new ContentValues();
                values.put(COLUMN_CLIENT_NAME, client);
                values.put(COLUMN_TEMPLATE_NAME, name);
    ...
    public void changePos(int projId, int  aId, int iId, int child, int catId, String dir, int branchcat){
                ...
                selectQuery = "SELECT  " + COLUMN_INSPECTION_ID + " FROM "
                        + TABLE_INSPECTION_ITEM + " WHERE " + COLUMN_PROJECT_ID + " = " + projId + " AND " + COLUMN_A_ID + " = " + aId;
                            ...
                            if (cursor.moveToFirst()) {
                                ...
                                //addLevel places a new label and corresponding aId in map.
 ...
 */
    ...
    public String getBranchPhoto(int projId, int aId) {
        SQLiteDatabase database = this.getWritableDatabase();
        String photo = "";

        String selectQuery = "SELECT " + COLUMN_IMG1
                + " FROM " + TABLE_MAP
                + " WHERE " + COLUMN_PROJECT_ID + " = " + projId + " AND " + COLUMN_A_ID + " = " + aId;
    ...
    public String getFolderPhoto(int projId) {
        SQLiteDatabase database = this.getWritableDatabase();
        String photo = "";

        String selectQuery = "SELECT " + COLUMN_PROJECT_PHOTO
    ...
    public HashMap<String, String> getInspectionData(int projId, int iId) {
        HashMap<String, String> inspectionsMap = new HashMap<String, String>();

        ArrayList<HashMap<String, String>> inspectionList;
        ...
        String selectQuery = "SELECT  * FROM "+ TABLE_INSPECTION
                + " WHERE "+ COLUMN_PROJECT_ID + " = "+projId+" AND "
                +  COLUMN_INSPECTION_ID+" = "+ iId //+" AND "+ COLUMN_PARENT+" > 0"
                ;
        Cursor cursor = dtabase.rawQuery(selectQuery, null);
        // Move to the first row
        if (cursor.moveToFirst()) {
            do {
                ...
                inspectionsMap.put(MyConfig.TAG_NOTE, cursor.getString(13));
                inspectionsMap.put(MyConfig.TAG_NOTE_2, cursor.getString(14));
                inspectionList.add(inspectionsMap);
    ...
    //Select inspection items for syncing to server
    public ArrayList<HashMap<String, String>> getInspectedItems_r(int user_id) {
    ...
    public ArrayList<HashMap<String, String>> getFolder(int user_id, int folderId) {
        ...
        String selectQuery = "SELECT M."+COLUMN_PROJECT_ID + ", M." +  COLUMN_CAT_ID + ", I." + COLUMN_LEVEL + ", I." + COLUMN_PARENT
                ...
                + " JOIN " + TABLE_INSPECTION + " I"
                + " ON M." + COLUMN_PROJECT_ID + " = I."+ COLUMN_PROJECT_ID
                + " WHERE I."+COLUMN_INSPECTOR+" = "+user_id + " AND M." + COLUMN_PROJECT_ID + " = "+ folderId + " AND M." + COLUMN_CAT_ID + " = " + 0 + " ORDER BY I." + COLUMN_INSPECTION_ID ;
    ...
    //Get a list of sublocations to populate the sub location spinner
    public ArrayList<HashMap<String, String>> getProjects(int user_id, int folder_id, int Level, int catId, int aId) {
            ...
            do {
                ...
                if (cursorB.moveToFirst()) {
                    do {
                        ...
                        int parentId = cursorB.getInt(16);
                        ...
                        String selectQueryC = "SELECT  I." + COLUMN_OVERVIEW + ", I." + COLUMN_COM1 + ", I." + COLUMN_RELEVANT_INFO
                                ...
                                " AND M."+COLUMN_A_ID+" = "+ cursorB.getString(17) //this is aID of the MAP table
                                + " ORDER BY I." + COLUMN_A_ID;
